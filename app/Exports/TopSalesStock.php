...
class TopSalesStock implements FromCollection, WithHeadings, ShouldAutoSize, WithColumnFormatting, WithEvents
{
    ...
    */
    ...
    public function __construct($data){
        $this->data = $data;
        $this->records =  collect(DB::select("SELECT SUM(order_items.quantity) as totalQty, SUM((order_items.price)*(order_items.quantity)) as totalAmount, SUM(? * order_items.quantity) as cost_amount, SUM(order_items.amount - (? * order_items.quantity)) as gross_profit, COUNT(distinct(order_items.price)) as totalRecords from order_items JOIN orders ON order_items.order_id = orders.order_id where orders.store_id = ? AND (date(order_items.created_at) between ? AND ?) AND orders.order_status !='CANCLED' AND orders.qty_id = ? ", [$this->data['costPrice'], $this->data['costPrice'], $this->data['storeId'], $this->data['fromDate'], $this->data['toDate'], $this->data['qtyId'] ] ))->first();
        $this->reportResult = DB::select("SELECT items.item_name, qty_types.qty_desc, SUM(order_items.quantity) as totalQty, SUM((order_items.price)*(order_items.quantity)) as amount, SUM(? * order_items.quantity) as cost_amount, SUM(order_items.amount - (? * order_items.quantity)) as gross_profit FROM order_items JOIN orders ON order_items.order_id = orders.order_id JOIN items ON items.item_id = order_items.item_id JOIN qty_types ON orders.qty_id = qty_types.qty_id where orders.store_id = ? AND ( date(order_items.created_at) between ? AND ? ) AND orders.order_status !='CANCLED' AND orders.qty_id = ? GROUP BY order_items.item_id  ORDER BY items.item_name", [$this->data['costPrice'], $this->data['costPrice'], $this->data['storeId'], $this->data['fromDate'], $this->data['toDate'], $this->data['qtyId'] ]);
