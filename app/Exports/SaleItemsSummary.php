...
class SaleItemsSummary implements FromCollection, WithHeadings, ShouldAutoSize, WithColumnFormatting, WithEvents
{
    use Exportable;
    ...
    */
    ...
    public function __construct($data){
        $this->data = $data;
        $this->reportResult = DB::select("SELECT orders.order_date, items.item_name, qty_types.qty_desc, order_items.quantity as volume, (order_items.price * order_items.quantity) as amount from order_items join items on items.item_id = order_items.item_id join orders on orders.order_id = order_items.order_id join qty_types on qty_types.qty_id = orders.qty_id where  orders.store_id = ? AND (orders.order_date between ? AND ?) AND orders.order_status !='CANCLED' AND orders.qty_id = ? ORDER BY orders.order_date", [ $this->data['storeId'], $this->data['fromDate'], $this->data['toDate'], $this->data['qtyId'] ]);
