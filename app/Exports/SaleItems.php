...
class SaleItems implements FromCollection, WithHeadings, ShouldAutoSize, WithColumnFormatting, WithEvents
{
    ...
    */
    ...
    public function __construct($data){
        $this->data = $data;
        $this->reportResult = DB::select("SELECT orders.order_date, orders.order_no, customers.cus_name, users.name, items.item_name, qty_types.qty_desc,  order_items.price, order_items.quantity, (order_items.price * order_items.quantity) as amount, (".$this->data['costPrice']." * order_items.quantity) as cost, ((order_items.price * order_items.quantity) - (".$this->data['costPrice']." * order_items.quantity)) as gross_profit  FROM orders join order_items on orders.order_id = order_items.order_id join items on items.item_id = order_items.item_id join customers on customers.cus_id = orders.cus_id join users on users.user_id = orders.user_id join qty_types on qty_types.qty_id = orders.qty_id where  orders.store_id = ? AND (orders.order_date between ? AND ?) AND orders.order_status !='CANCLED' AND orders.qty_id = ? ORDER BY orders.order_date", [$this->data['storeId'], $this->data['fromDate'], $this->data['toDate'], $this->data['qtyId'] ]);
